          ...
          100 *
            purchases.reduce(
              (sum, item) =>
                sum +
                item.product.prices[
                  item.product.prices.findIndex(currencyFilter)
                ].amount *
                  item.amount,
              0
            ),
                ...
                return (
                  <div className="cartItem" key={index}>
                    <div className="cartItemParams">
                      ...
                      <div className="cartPriceParam">
                        {this.props.currency}
                        {purchase.product.prices &&
                          purchase.product.prices[
                            purchase.product.prices.findIndex(currencyFilter)
                          ].amount}
                      </div>
                      {purchase.product.attributes &&
                        purchase.product.attributes.map(
                          (attribute, cartIndex) => (
                            <label
                              className="cartAttributeName"
                              key={cartIndex}
                            >
                              {attribute.name + ":"}
                              <div>
                                {attribute.items.map((item, attrIndex) =>
                                  attribute.type !== "swatch" ? (
                                    <div
                                      className={
                                        purchase.params[attribute.name] ===
                                        item.value
              ...
              <div className="totalPrice">
                ...
                {this.props.purchases && (
                  <span className="totalPriceSum">
                    {this.props.currency}
                    {totalPrice(this.props.purchases)}
                  </span>
                )}
              </div>
